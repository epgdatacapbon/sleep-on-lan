...
func ListenerUDP(port int) {
	...
	if err != nil {
		logger.Error("Error while resolving local address: " + err.Error())
		os.Exit(1)
	}
...
func ReadPacket(sock *net.UDPConn) {
	...
	for {
		...
		if err == nil {
			...
			logger.Warning("Error while reading a packet: " + err.Error())
		}
	}
}
...
func extractMacAddress(rlen int, buf [1024]byte) (net.HardwareAddr, error) {
	...
	if rlen >= 12 {
		...
		logger.Warning("The received packet is too small, size [" + strconv.Itoa(rlen) + "]")
	}
